...
def build_dagster_type_namespace_snapshot(pipeline_def):
    ...
    return DagsterTypeNamespaceSnapshot(
        {dt.key: build_dagster_type_meta(dt) for dt in pipeline_def.all_dagster_types()}
    )


def build_dagster_type_meta(dagster_type):
    check.inst_param(dagster_type, 'dagster_type', DagsterType)
    return DagsterTypeMeta(
...
class DagsterTypeNamespaceSnapshot(
    namedtuple('_DagsterTypeNamespaceSnapshot', 'all_dagster_type_metas_by_key')
):
    def __new__(cls, all_dagster_type_metas_by_key):
        return super(DagsterTypeNamespaceSnapshot, cls).__new__(
            cls,
            all_dagster_type_metas_by_key=check.dict_param(
                all_dagster_type_metas_by_key,
                'all_dagster_type_metas_by_key',
                key_type=str,
                value_type=DagsterTypeMeta,
            ),
        )

    def get_dagster_type_meta(self, key):
        check.str_param(key, 'key')
        return self.all_dagster_type_metas_by_key[key]


@whitelist_for_serdes
class DagsterTypeMeta(
    namedtuple(
        '_DagsterTypeMeta',
        'kind key name description '
        'is_builtin '
        'type_param_keys '
    ...
    def __new__(
        ...
        return super(DagsterTypeMeta, cls).__new__(
            cls,
