...
class HDFSFileHandle(FileHandle):
    def __init__(self, hdfs_prefix, path):
        self._hdfs_prefix = check.str_param(hdfs_prefix, 'hdfs_prefix')
    ...
    @property
    def hdfs_path(self):
        return '%s/%s' % (self.hdfs_prefix, self.path)


class HDFSFileManager(FileManager):
    def __init__(self, hdfs_fs, hdfs_prefix):
    ...
    def _download_if_not_cached(self, file_handle):
        if not self._file_handle_cached(file_handle):
            ...
            self.hdfs_fs.download(file_handle.hdfs_path, temp_file_obj)
    ...
    def write(self, file_obj, mode='wb'):
        ...
        fh = HDFSFileHandle(self._hdfs_prefix, str(uuid.uuid4()))
        self.hdfs_fs.upload(fh.hdfs_path, file_obj)
        return fh
