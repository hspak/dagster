...
import pytest
from dagster_hadoop.hdfs import HDFSIntermediateStore, hdfs_plus_default_storage_defs, hdfs_resource
...
def define_inty_pipeline(should_throw=True):
    ...
    @pipeline(
        mode_defs=[
            ModeDefinition(
                system_storage_defs=hdfs_plus_default_storage_defs,
                resource_defs={'hdfs': hdfs_resource},
            )
        ]
    )
...
@nettest
def test_using_hdfs_for_subplan():
    pipeline_def = define_inty_pipeline()

    environment_dict = {'storage': {'hdfs': {}}}
    ...
    with scoped_pipeline_context(
        ...
        store = HDFSIntermediateStore(
            run_id,
            hdfs_fs=context.scoped_resources_builder.build(
...
class FancyStringHDFSTypeStoragePlugin(TypeStoragePlugin):  # pylint:disable=no-init
    @classmethod
    ...
    def set_object(cls, intermediate_store, obj, context, dagster_type, paths):
        check.inst_param(intermediate_store, 'intermediate_store', HDFSIntermediateStore)
        paths.append(obj)
    ...
    def get_object(cls, intermediate_store, _context, _dagster_type, paths):
        check.inst_param(intermediate_store, 'intermediate_store', HDFSIntermediateStore)
        res = list(
            intermediate_store.object_store.hdfs_fs.list_blobs(
...
@nettest
def test_hdfs_intermediate_store_with_type_storage_plugin():
    run_id = make_new_run_id()

    intermediate_store = HDFSIntermediateStore(
        run_id=run_id,
        type_storage_plugin_registry=TypeStoragePluginRegistry(
            [(RuntimeString, FancyStringHDFSTypeStoragePlugin)]
        ),
    )
...
@nettest
def test_hdfs_intermediate_store_with_composite_type_storage_plugin():
    run_id = make_new_run_id()
...
@nettest
def test_hdfs_intermediate_store_composite_types_with_custom_serializer_for_inner_type():
    run_id = make_new_run_id()

    intermediate_store = HDFSIntermediateStore(run_id=run_id)
...
@nettest
def test_hdfs_intermediate_store_with_custom_serializer():
    run_id = make_new_run_id()
    ...
    with yield_empty_pipeline_context(run_id=run_id) as context:
        try:
            ...
            bucket_obj = intermediate_store.object_store.hdfs_prefix.get_bucket(
                intermediate_store.object_store.bucket
            )
...
@nettest
def test_hdfs_pipeline_with_custom_prefix():
    run_id = make_new_run_id()
    hdfs_prefix = 'custom_prefix'
    ...
    with scoped_pipeline_context(
        ...
        store = HDFSIntermediateStore(
            run_id=run_id,
            hdfs_fs=context.scoped_resources_builder.build(required_resource_keys={'hdfs'}).hdfs.fs,
...
@nettest
def test_hdfs_intermediate_store_with_custom_prefix():
    run_id = make_new_run_id()

    intermediate_store = HDFSIntermediateStore(run_id=run_id, hdfs_prefix='custom_prefix')
    ...
    try:
        with yield_empty_pipeline_context(run_id=run_id) as context:
            ...
            assert intermediate_store.has_object(context, [obj_name])
            assert intermediate_store.uri_for_paths([obj_name]).startswith('hdfs://custom_prefix')
...
@nettest
def test_hdfs_intermediate_store():
    run_id = make_new_run_id()
    ...
    intermediate_store_2 = HDFSIntermediateStore(run_id=run_id_2)
...
def test_custom_read_write_mode():
    run_id = make_new_run_id()
    intermediate_store = HDFSIntermediateStore(run_id=run_id)
