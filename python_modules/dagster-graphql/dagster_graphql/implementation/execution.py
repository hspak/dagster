...
def start_scheduled_execution(graphene_info, schedule_name):
    ...
    try:
        ...
        tick = graphene_info.context.instance.create_schedule_tick(
            graphene_info.context.get_repository(),
            ScheduleTickData(schedule_def.name, ScheduleTickStatus.STARTED),
        )
        ...
        if not should_execute:
            ...
            graphene_info.context.instance.update_schedule_tick(
                graphene_info.context.get_repository(), tick
            )
        ...
        run = result.run
        tick = tick.with_status(ScheduleTickStatus.SUCCESS, ScheduleTickSuccessData(run.run_id))
        graphene_info.context.instance.update_schedule_tick(
            graphene_info.context.get_repository(), tick
        )
        ...
        tick = tick.with_status(ScheduleTickStatus.FAILURE, ScheduleTickFailureData(error=error_data))
        graphene_info.context.instance.update_schedule_tick(
            graphene_info.context.get_repository(), tick
        )
