...
class DauphinEvaluationStackEntry(dauphin.Union):
    ...
    @staticmethod
    # TODO remove config_schema_snapshot
    def from_native_entry(entry, config_schema_snapshot):
        check.inst_param(config_schema_snapshot, 'config_schema_snapshot', ConfigSchemaSnapshot)
...
class DauphinEvaluationStack(dauphin.ObjectType):
    ...
    def resolve_entries(self, graphene_info):
        return map(
            lambda e: graphene_info.schema.type_named('EvaluationStackEntry').from_native_entry(
                e, self._config_schema_snapshot
            ),
            self._entries,
        )
